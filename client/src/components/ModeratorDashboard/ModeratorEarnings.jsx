...
const ModeratorEarnings = () => {
  ...
  const categoryEarnings = data?.categoryEarnings || [];
  const dailyTrends      = data?.dailyTrends      || [];
  const paymentStatus    = data?.paymentStatus    || { received: 0, pending: 0 };
  const topHelpers       = data?.topHelpers       || [];

  const hasData = monthlyEarnings.length > 0 || dailyTrends.length > 0;

  // Summary metrics
  const totalEarnings        = monthlyEarnings.reduce((sum, e) => sum + e.amount, 0);
  const totalBookings        = dailyTrends.reduce ? dailyTrends.length : 0;
  ...
  const conversionRate       = (receivedPayments + pendingPayments) > 0
    ? ((receivedPayments / (receivedPayments + pendingPayments)) * 100).toFixed(1)
    : 0;
    ...
    <div className={styles.container}>
      ...
      <div className={styles.summaryGrid}>
        <div className={styles.summaryCard}>
          <div className={styles.cardContent}>
            <h3 className={styles.cardValue}>&#8377;{totalEarnings.toLocaleString('en-IN')}</h3>
        ...
        <div className={styles.summaryCard}>
          <div className={styles.cardContent}>
            ...
            <p className={styles.cardLabel}>Bookings with Revenue</p>
          </div>
        </div>
        <div className={styles.summaryCard}>
          <div className={styles.cardContent}>
            <h3 className={styles.cardValue}>&#8377;{Math.round(averageBookingValue).toLocaleString('en-IN')}</h3>
      ...
      {!hasData ? (
        <div className={styles.emptyState}>
          ...
          <p>Analytics will appear here once bookings with payments are recorded in your location.</p>
        </div>
      ) : (
        <>
          {/* Charts Grid */}
            ...
            <div className={styles.chartCard}>
              ...
              <div className={styles.paymentStats}>
                <div className={styles.statItem}>
                  ...
                  <span>Received: &#8377;{receivedPayments.toLocaleString('en-IN')}</span>
                </div>
                <div className={styles.statItem}>
                  <span className={styles.statDot} style={{ backgroundColor: "#ff9800" }}></span>
                  <span>Pending: &#8377;{pendingPayments.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </div>
